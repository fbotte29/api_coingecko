...
@task
def request():
    response_5 = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin%2Csolana%2Clitecoin%2Cethereum&vs_currencies=eur%2Cusd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true&include_last_updated_at=true&precision=2")
...
def parse_api_coingecko(simple_price):
    ...
    return eur, usd, eur_24h_vol, eur_market_cap, last_updated_at,sol_eur,sol_usd, sol_eur_24h_vol,sol_eur_24h_change,sol_eur_market_cap,sol_last_updated_at, ltc_eur, ltc_usd, ltc_eur_24h_vol, ltc_eur_market_cap, ltc_last_updated_at, eth_eur, eth_usd, eth_eur_24h_vol, eth_eur_market_cap, eth_last_updated_at

@task(viz_return_value=[1, 2, 3])
def transfo(last_updated_at,sol_last_updated_at,ltc_last_updated_at,eth_last_updated_at):
    last_update_1 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(last_updated_at))
    sol_last_updated = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ltc_last_updated_at))
    ...
    return last_update_1,sol_last_updated,ltc_last_updated,eth_last_updated


## INSERT
# CONNEXION BDD
@task 
def insert(eur,usd,last_update_1,eur_market_cap,eur_24h_vol,sol_eur,sol_usd,sol_eur_24h_vol,sol_eur_24h_change,sol_eur_market_cap,sol_last_updated_at, ltc_eur,ltc_usd,ltc_eur_24h_vol,ltc_eur_market_cap,ltc_last_updated,eth_eur,eth_usd,eth_eur_24h_vol,eth_eur_market_cap,eth_last_updated):
    ...
    req_sol = 'INSERT INTO sol(eur,usd,last_update,eur_market_cap,eur_24h_vol) VALUES(?,?,?,?,?)'
    val_sol = (sol_eur,sol_usd,sol_last_update,sol_eur_market_cap,sol_eur_24h_vol)
...
#fig.show()
    ...
    fig.write_html("/home/FBO/git/Api/api_coingecko/visu/ltc_eur_usd_DEX_24h.html")
